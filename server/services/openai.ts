...
function getBotSystemPrompt(botId: string, section: string): string {
  ...
  const botPrompts: Record<string, string> = {
    ...
    'creative-concepts': `${baseBehavior} You generate innovative advertising concepts. Brainstorm creative ideas that capture attention, communicate value propositions, and inspire desired actions.`,
    'audience-targeting': `${baseBehavior} You develop precise audience targeting strategies. Analyze demographics, psychographics, and behaviors to identify optimal target segments for advertising campaigns.`,
    'budget-optimizer': `${baseBehavior} You optimize advertising budgets for maximum ROI. Analyze performance data, recommend budget allocation across channels, and suggest bid strategies.`,
    'ab-testing': `${baseBehavior} You design A/B testing strategies for ads. Create test hypotheses, design experiments, and provide frameworks for analyzing results and implementing improvements.`,
    'landing-pages': `${baseBehavior} You design high-converting landing pages. Create page structures, copy, and call-to-action strategies that maximize conversion rates from advertising traffic.`,
